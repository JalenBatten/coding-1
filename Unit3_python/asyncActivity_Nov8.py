string_concatenation= 'A string concatenation is adding strings together.'
x=3
y=2
z= x+y
print(z)
rule1= 'A variable name must start with a letter or an underscore.' 
rule2= ' A variable name should be short, simple, yet descriptive.'
rule3= ' Variable names cannot contain any spaces.'
rulesOfVariables= rule1 + rule2 + rule3
print(rulesOfVariables)

camelCaseExample= 'In camel case every word starts with a captital letter, except the first letter.' 
PascalCaseExample= 'In pascal case every word starts with a captial letter, including the first letter'
snake_case_example='To separate words in snake case underscores are used.'
 
a=3
b=a*100
print(b)

x=10
y=2
z=y*2



